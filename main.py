...
def main():
    ...
    print(
        'Naive Homography Slow computation takes {:5.4f} sec'.format(toc(tt)))
    plt.figure()
    ...
    print(
        'Naive Homography Fast computation takes {:5.4f} sec'.format(toc(tt)))
    plt.figure()
    ...
    plt.figure()
    forward_panorama_imperfect_matches_plot = plt.imshow(
        transformed_image_fast)
    ...
    # Plot naive  RANSAC homography with forward mapping
    tt = time.time()
    transformed_image_fast_RANSAC = solution.compute_forward_homography_fast(
        homography=ransac_homography,
        src_image=src_img,
        dst_image_shape=dst_img.shape)

    print('Naive Homography Fast computation for imperfect matches after RANSAC takes '
          '{:5.4f} sec'.format(toc(tt)))
    plt.figure()
    forward_panorama_imperfect_matches_plot = plt.imshow(
        transformed_image_fast_RANSAC)
    plt.title('Forward Panorama RANSAC imperfect matches')
    # plt.show()
